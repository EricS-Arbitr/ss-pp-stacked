...
# ansible.builtin.fail. That would have fired the fail task on every host in
# the play -- a targeted guard turned into a range-wide outage. The file was
# valid YAML with no duplicate keys and balanced quotes, so neither of the
# checks above could see it, and it reads correctly at a glance: both lines
# spelled right, only the column wrong.
